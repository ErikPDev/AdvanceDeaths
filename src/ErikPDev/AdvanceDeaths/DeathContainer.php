...
class DeathContainer
{
    ...
    public function Translate($translate, $entity)
    {
        ...
        foreach ($KeyWordsFound[0] as $value => $KeyWord) {
            if ($entity->getLastDamageCause() instanceof EntityDamageByEntityEvent) {
                if ($KeyWord == "{killer_kills}" || $KeyWord == "{killer_deaths}" || $KeyWord == "{player_kills}" || $KeyWord == "{player_deaths}" || $KeyWord == "{player_kdr}" || $KeyWord == "{killer_kdr}" || $KeyWord == "{killer_killstreak}") {
                    continue;
                }
            }
    ...
    /**
     * Private function to broadcast a message.
     * @param $message
     * @return void
     */
